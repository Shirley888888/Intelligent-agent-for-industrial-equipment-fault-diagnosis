...
>
> **新切分方式已统一重跑完成（r8，2026-09-01）**：先按时间把原始序列切成 70/15/15、
> 再各段内滑窗；5 个模型各 200 epochs 全量训练。r8 结果与 final Test 正式评价
> 见第 4 节、第 5 节；第 6 节结论已按新结果更新。
...
- **r8 为新切分方式的正式重跑**：5 个模型各 200 epochs 全量训练，作为最终结论依据
...
### 5.1 各模型 r8 Validation 表现（模型选择依据）
...
| linear   | r8         | Linear(672→24)                          |      1.2983  |      2.8719  |  16,152  |     200    |        58.45 |
| mlp      | r8         | MLP(672→128×2层→24)                     |      2.9186  |     12.7313  | 105,752  |      25    |       112.96 |
| cnn      | r8         | CNN(kernel=5, channels=32)              |      2.4663  |      9.5671  |   1,944  |      19    |        89.67 |
| lstm     | r8         | LSTM(2层, hidden=64)                    |      1.9498  |      6.0856  |  53,528  |      36    |      1929.43 |
...
## 6. 结论与讨论（基于新切分 r8 与 final Test 正式评价）

> 旧切分 r1–r7 的结构变化 / 超参补测结论（旧第 6 节）已随归档失效，此处不再展开；
> 新切分下的结构变化 / 超参消融正式重跑尚未补齐，属后续工作。当前结论以 r8（单配置正式重跑）+ final Test 为准。

### 最终结论

1. **最终方案：Linear(672→24)，Test MAE=1.3788、MSE=3.5725**，相比 Last-Value Baseline（Test MAE=1.4597）提升约 **5.5%**。按"Validation 选型"原则，r8 中 Baseline（1.2839）与 Linear（1.2983）在 Validation 上几乎持平，取学习模型中最优的 Linear 作为最终方案，Test 上仅做一次正式评价。
2. **新切分纠正了旧结论**：旧切分（训练/验证样本重叠）曾得出"LSTM 最优（MAE=1.3658）"；新切分下 LSTM 的 Validation MAE=1.9498，明显劣于 Baseline/Linear，且训练耗时 1929s 为全实验最高。说明旧切分因边界样本重叠**高估了深模型**，新流程下的结果更可信。
3. **Linear 是稳健首选**：零过拟合（best_epoch=200 仍在下降）、训练仅 58 秒（LSTM 的 1/33）、参数量 1.6 万。在 ETTh1 这类强自相关的电力数据上，短期（24h）油温预测以线性外推为主，深模型的时序建模优势被压缩。
4. **MLP/CNN/LSTM 均劣于 Baseline（Validation）**：MLP val MAE=2.9186（best_epoch=25）、CNN 2.4663（best_epoch=19）、LSTM 1.9498（best_epoch=36），三者 best_epoch 远早于 200，过拟合普遍。在 1.2 万窗口的小样本下，深模型必须配合早停，否则越训越差。
5. **工程建议**：当前配置下 **Linear（bs=128, lr=1e-4, 200ep）** 精度最高且零调参风险；若后续迭代结构/超参消融（新切分），应优先关注 Linear 的扩展（如增加特征交叉、更多轮数）及深模型的早停策略。
