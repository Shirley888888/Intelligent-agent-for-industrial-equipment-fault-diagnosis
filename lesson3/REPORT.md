...
## 5. 统一结果表与曲线（旧切分方式存档；新流程见第 0 节）

> **本节省略提示**：本节 r1–r7 统一表为**旧切分方式**存档。新流程下 `summary.py`
> 改为基于 **Validation 指标**（`val_mae/val_mse`）生成模型选择表
> （`outputs/unified_test_table.md`），最终方案确定后由 `evaluate.py`
> 在 Test 上正式评价（`outputs/final_test_table.md`），待重跑后本报告将据此更新。
